          ...
          <div key={order.id} className="border border-zinc-100 rounded-3xl overflow-hidden transition-all bg-white shadow-sm hover:shadow-md">
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">{t("orders_order_id")}</span>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">{t("orders_total_amount")}</span>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="text-purple-500 flex items-center gap-1 text-[10px] font-bold uppercase">
                    ...
                    {isExpanded ? t("orders_hide") : t("orders_details")}
                  </div>
                </div>
              </div>
            </div>
            ...
            {isExpanded && (
              <div className="bg-zinc-50/50 border-t border-zinc-100 p-6 space-y-6 animate-in fade-in slide-in-from-top-1 duration-200">
                <div>
                  <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">{t("orders_items_purchased")}</h4>
                ...
                <div className="grid md:grid-cols-2 gap-6 pt-6 border-t border-zinc-200">
                  <div>
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">{t("orders_shipping_details")}</h4>
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">{t("orders_order_summary")}</h4>
                    <div className="bg-white p-4 rounded-2xl border border-zinc-100 space-y-2">
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500">{t("orders_subtotal")}</span>
                        <span className="font-medium text-zinc-800">EGP {order.totals?.subtotal}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500">{t("orders_shipping")}</span>
                      ...
                      <div className="flex justify-between text-sm pt-2 border-t border-dashed border-zinc-100 mt-2">
                        <span className="font-bold text-zinc-800">{t("orders_total_paid")}</span>
                        <span className="font-bold text-purple-600 text-base">EGP {order.totals?.total}</span>
                      </div>
                      <div className="pt-2">
                        <span className="text-[10px] bg-zinc-50 text-zinc-400 px-2 py-1 rounded block text-center">
                          {t("orders_payment_prefix")} {order.payment?.method === 'cod' ? t("orders_payment_cod") : order.payment?.method}
                        </span>
                      </div>
                    </div>
                  </div>
